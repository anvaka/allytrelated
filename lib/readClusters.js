...
function readClsuters(dirName, done) {
  ...
  function api() {
    return {
      ...
      getSize: getSize,
      info: info,
      getAllByNodeData: getAllByNodeData,
      findNodeByData: findNodeByData,
      getNodeClusterGraph: getNodeClusterGraph
    };
  }

  function getNodeClusterGraph(data) {
    var info = getAllByNodeData(data);
    if (!info) return;
    var graph = createGraph({uniqueLinkId: false});

    info.nodes.forEach(addNodes);
    info.nodes.forEach(addLinks);
    ...
    function addLink(node, link) {
      ...
      console.log('checking ' + fromId + ' -> ' + toId);

      if (!graph.getNode(fromId)) return;
      console.log('Graph has fromId ' + fromId);

      if (!graph.getNode(toId)) return;
      console.log('Graph has toId ' + toId);
      graph.addLink(fromId, toId);
    }
  }


  function getAllByNodeData(data) {
    var nodeId = findNodeByData(data);
    if (nodeId === undefined) return;

    var c = getNodeCluster(nodeId);
    return info(c, Number.POSITIVE_INFINITY);
  }
  ...
  function info(clusterId, maxVideo) {
