...
function loadGraph(fileName, done, addData) {
  addData = typeof addData === 'function' ? addData : noop;
  ...
  function addChannel(pkg) {
    ...
    var data = addData(pkg);

    if (data !== undefined) graph.addNode(pkg.id, data);
    else graph.addNode(pkg.id);
